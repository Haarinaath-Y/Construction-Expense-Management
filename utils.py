...
def purchase_amounts(project_id, database_name):
    try:
        with connect_db(database_name) as conn:
            cursor = conn.cursor()
            # Fetch distinct categories from the category table
            ...
            sql_query += f" FROM purchases p WHERE project_id = {project_id} GROUP BY p.category"
...
def create_vendor_bar_chart(project_id, db_name):
    """
    Creates a bar chart showing the total purchase amount by vendor for a specific project.

    Args:
    project_id (int): The project ID to filter purchases by.
    db_path (str): Path to the SQLite database file (default: 'your_database.db').
    """

    with connect_db(db_name) as conn:
        # SQL query to get purchase amount by vendor for the selected project
        query = f"""
            SELECT 
                vendor as 'Vendor',
                SUM(p.purchase_amount) as 'Purchase Amount'
            FROM purchases p
    ...
    if not df.empty:
        # Display the bar chart using Altair for customization
        bar_chart = alt.Chart(df).mark_bar(color='#FF4B4B').encode(
            y=alt.Y('Vendor', sort=None),
            x='Purchase Amount'
        )
        ...
        text = bar_chart.mark_text(
            ...
            baseline='middle',
            color='white',
            dx=3  # Adjust the position of the labels
        ).encode(
            text='Purchase Amount:Q'  # Display the value of the Purchase Amount
        )
