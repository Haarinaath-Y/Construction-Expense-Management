...
from connection_utils import (upload_db_to_drive, share_file_with_user, check_existing_file, delete_files_with_db_name)
...
def purchase_entry(service, db_name, project, project_decision):
    ...
    if project and project_decision == 'Select Existing Project':
        ...
        if project_selection != "Project Names with Project ID":
            ...
            if st.button("View Purchases", icon=':material/table_view:'):
                with connect_db(db_name) as conn:
                    cursor = conn.cursor()
                    cursor.execute(f'''
                            SELECT 
